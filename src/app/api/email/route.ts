...
function validate(body: Record<string, unknown>): { error: string } | { data: Record<keyof typeof MAX_LENGTHS, string> } {
  ...
  for (const field of Object.keys(MAX_LENGTHS) as (keyof typeof MAX_LENGTHS)[]) {
    ...
    if (typeof value !== 'string' || !value.trim()) {
      return { error: 'Tous les champs (nom, email, sujet, message) sont obligatoires.' };
    }
...
export async function POST(request: NextRequest) {
  ...
  if (!EMAIL_HOST || !EMAIL_USER || !EMAIL_PASS) {
    console.error("Variables d'environnement SMTP_HOST, SMTP_USER ou SMTP_PASS manquantes.");
